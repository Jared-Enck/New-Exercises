...
class BinaryTree {
  ...
  nextLarger(
    ...
    if (!current) {
      if (!higherBoundVals.length) {
        return null
      }
      return Math.min(...higherBoundVals);
    }

    if (current.val > lowerBound) higherBoundVals.push(current.val);
    if (current.left) toVisitStack.push(current.left)
    if (current.right) toVisitStack.push(current.right)

    return this.nextLarger(lowerBound, higherBoundVals, toVisitStack)
  }
