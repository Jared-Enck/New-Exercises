...
describe('POST /users', () => {
    test('Creates a user', async () => {
        ...
        expect(res.body).toEqual({ user: {id: expect.any(Number), name: 'BillyBob', type: 'staff'}})
    })
})
...
describe('PATCH /users/:id', () => {
    test('Updates a user', async () => {
        ...
        expect(res.statusCode).toBe(200)
        expect(res.body).toEqual({ user: {id: testUser.id, name: 'BillyBob', type: 'admin'}})
    })
    test('Responds with 404 if updating user with invalid id', async () => {
...
describe('DELETE /users/:id', () => {
    ...
    test('Responds with 404 if deleting user with invalid id', async () => {
